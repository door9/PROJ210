...
import { uid, todayStr, esc, fmtMoney, fmtPct, fmtQty, pctClass, bindThousands, bindCap, numOf } from './util.js';
  ...
  const filterBar = `
    <div class="card" style="padding:10px 12px;">
      <div class="btn-row" style="margin:0; flex-wrap:wrap; align-items:center;">
        ...
        <select data-symsel class="ctl-s" style="margin-left:auto; max-width:190px;">
...
function openSwapModal() {
  ...
  const m = openModal(`
    <h2>교체 시뮬레이션</h2>
    <p class="small muted" data-note style="margin-top:-6px;">"이걸 팔아 저걸 샀다면"을 저장해 두면, 하지 않은 선택의 성적을 계속 채점해 줍니다.</p>
    <form id="swap-form">
      <div class="form-grid">
...
function openLoanModal(existing) {
  ...
  const m = openModal(`
    ...
    <p class="small muted" data-note style="margin-top:-6px;">대출 계좌마다 한 건씩 등록하세요. 각 계좌는 시작일부터 (상환일 또는) 지금까지 따로따로 이자가 쌓입니다.</p>
    <form id="loan-form">
      <div class="form-grid">
...
function openRepayModal(loanId) {
  ...
  const m = openModal(`
    ...
    <form id="rep-form">
      <div class="form-grid">
        ...
        <label class="fld">갚은 금액 (원)<input name="amount" type="text" inputmode="decimal" autocomplete="off" required>
          <span class="muted small" data-cap style="display:none; margin-top:4px; font-weight:400;"></span></label>
        <label class="fld full">메모 (선택)<input name="note" maxlength="60" placeholder="예: 보너스로 일부 상환"></label>
      </div>
      <p class="hint" style="margin:2px 0 0;">전액을 갚았다면 '전액'을 누르세요 — 잔액이 0이 되면 그날로 이자가 멈춥니다.</p>
      <div class="btn-row" style="justify-content:flex-end;">
  ...
  bindThousands(amt);
  // 갚는 돈은 남은 대출 잔액까지만. 동기화로 대출이 바뀌었을 수 있어 그릴 때마다 다시 계산한다.
  const cap = bindCap(amt, m.querySelector('[data-cap]'), () => {
    const a = E.loanStatus(state)?.accounts.find(x => x.id === loanId);
    const left = Math.max(0, a ? a.balance : (find()?.balance || 0));
    return {
      max: left,
      basis: left > 0.5 ? '남은 대출 잔액' : '이미 다 갚은 대출입니다',
      fmt: v => fmtMoney(Math.floor(v + 1e-6)),
      slack: 0.5,
      fill: v => Math.floor(v + 1e-6),
    };
  });
  ...
  m.querySelector('#rep-form').addEventListener('submit', e => {
    ...
    if (cap.over()) { toast(cap.message(), 4200); amt.focus(); return; }
...
function vCost() {
  ...
  return `
    ...
    <div class="kpis">
      ...
      <div class="kpi"><div class="k">명목 손익</div><div class="v ${pctClass(ln.profit)}">${fmtMoney(ln.profit)}</div><div class="s">이자 반영 전${ln.feeCut ? ' · 수수료·세금 차감' : ''}</div></div>
      <div class="kpi"><div class="k">이자 차감 후 실질 손익</div><div class="v ${pctClass(ln.netProfit)}">${fmtMoney(ln.netProfit)}</div><div class="s">명목 손익 − 누적 이자${ln.feeCut ? ' (매도 수수료·세금 −' + fmtMoney(ln.feeCut) + ' 반영)' : ''}</div></div>
    </div>
...
function openPeriodDetail(r) {
  ...
  const cashLine = pf.cashTracked
    ? `<p class="hint" data-keep>현금 ${fmtMoney(pf.cash.KRW)}${pf.cash.USD ? ' + ' + fmtMoney(pf.cash.USD, 'USD') : ''}
       <span class="muted">(${esc(pf.cashAsOf)} 입력분)</span> 포함</p>`
    : `<p class="hint" data-keep>이 시점엔 현금 입력이 없어 <b>보유 주식만</b> 집계했습니다.</p>`;
...
function vReturns() {
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="warnbox" data-note style="margin-top:10px;">
      ...
      <b>증권사 화면과 몇 가지 이유로 차이가 납니다.</b> 유상증자 신주는 신주인수권 매입가까지 원가에 넣고(실제로 쓴 돈이니까요),
      배당금·계좌이자는 위 표의 실현손익에 들어가지 않습니다 — 매매만 세기 때문입니다.</p>
    </div>`;
}
vReturns.bind_ = (root) => {
