...
function openCloseModal() {
  ...
  const m = openModal(`
    <h2>펀드 청산</h2>
    <p class="small muted" data-note style="margin-top:-6px;">
    ...
    <form id="close-fund">
      ...
      <p class="hint" data-keep style="margin:10px 0 0;">운용 기간은 <b>${esc(spanText(from, today))}</b>으로 기록됩니다(청산일을 바꾸면 그에 맞게).</p>
...
function vFunds() {
  ...
  const live = `
    <div class="card">
      ...
      ${hasRecords ? '' : '<p class="hint" data-keep>청산할 기록이 없습니다.</p>'}
    </div>`;
...
function vFund(id) {
  ...
  return `
    ...
    <p class="view-desc" data-keep>${genLabel(a.gen)} · ${esc(spanText(a.from, a.to))} · ${daysText(a.from, a.to)} 운용 · ${a.to}에 청산</p>
    ...
    <p class="small muted" data-note style="margin:6px 2px 14px;">청산일(${a.to}) 기준으로 얼린 값입니다.</p>` : ''}
    ...
    ${realRows ? `<div class="card">
      ...
      <p class="hint" data-keep>판 것만 집계합니다(원가 대비). 합계 ${fmtMoney(s.realizedPnlKRW || 0)} — 매도 시점 환율로 원화 환산.</p>
    </div>` : ''}
    <div class="card">
      <h3>남은 기록</h3>
