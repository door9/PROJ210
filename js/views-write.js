...
function openLetterEditor(period) {
  ...
  const m = openModal(`
    <h2>${period} 주주 서한</h2>
    <p class="small muted" data-note style="margin-top:-6px;">유일한 독자는 미래의 나. 잘한 것보다 부끄러운 것을 쓸 때 가치가 있습니다.</p>
  ...
  return `
    ...
    <div class="card">
      ...
      <p class="small muted" data-note style="margin:4px 0 0;">
        담기는 것: 펀드 현황 · 만약 결과 · 전체 매매(이유·확신도·감정 포함) · 실현 결과와 회상 · 홀딩 일지 · 지난 서한 전문 · 심문 지침.<br>
  ...
  return `
    ...
    <div class="card">
      ...
      <form id="name-form" class="btn-row" style="margin:0;">
        <input name="fundName" value="${esc(state.settings.fundName || 'PROJ210')}" class="ctl" style="flex:1;">
    ...
    <div class="card">
      ...
      <p class="small muted" data-note style="margin:4px 0 0;">내보낸 파일에는 <b>GitHub 토큰과 앱 잠금 PIN이 들어가지 않습니다</b> — 백업 사본이 돌아다녀도 안전합니다. 가져올 때는 이 기기에 저장된 값을 그대로 씁니다.</p>
      <p class="small muted" style="margin:10px 0 0;">
        <span data-note-anchor>이 기기의 앱 버전:</span> <b data-appver>확인 중…</b>
        <button class="btn small" data-x="forceupdate" style="margin-left:6px;">최신으로 갱신</button>
        <span class="muted" data-note>기기마다 버전이 다르면 한쪽이 옛 코드에 갇힌 것입니다 — 새 기능이 한쪽에서만 안 보이는 원인.</span>
      </p>
    </div>
    <div class="card">
      ...
      <p class="hint">새 종목을 기록하면 자동으로 등록을 요청합니다 — 시세는 1분 안에 저장소에 들어오고, 앱을 다시 열면 보입니다.<br>
      갱신: 각 시장이 <b>마감한 직후</b> 개인 서버가 수집을 요청해 종가를 받아 둡니다 —
      한국 15:33, 미국 05:05(서머타임) 또는 06:05(해제 시), 모두 한국시간. 주말·휴장일은 그 시장만 건너뜁니다.
      서버가 멈춰도 GitHub 예약 수집이 뒤를 받치고, 그래도 오늘 종가가 없으면 앱을 열 때 한 번 요청합니다.<br>
      앱은 기기에 받아 둔 시세로 바로 화면을 그리고 바뀐 종목만 새로 받습니다.
      지금 당장 받고 싶으면 위쪽 <b>↻</b> 버튼을 누르세요(20초 안팎).</p>
    </div>
    <div class="card">
      <h3>앱 잠금 (PIN)</h3>
      ${Lock.hasPin()
    ...
    <div class="card">
      <h3>현금 잔액</h3>
      <p class="small muted" data-note style="margin:4px 0 0;">매수·매도·입출금·환전·배당을 기록하면 잔액이 그만큼 자동으로 바뀝니다.
      증권사 잔액과 다르면 여기에 실제 잔액을 넣으세요 — 넣은 값이 기준이 됩니다.<br>
    ...
    <div class="card">
      ...
      <form id="rate-form" class="btn-row" style="align-items:center; gap:6px;">
        ...
        <input name="depositRate" type="number" step="0.1" min="0" max="20" inputmode="decimal"
          ...
          class="ctl-s" style="width:70px; text-align:right;">
...
vSettings.bind_ = (root) => {
  ...
  root.querySelector('[data-x=regpending]')?.addEventListener('click', async () => {
    ...
    toast(done ? `${done}개 등록 요청 완료 — 1분 안에 저장소에 들어오고, 앱을 다시 열면 보입니다` : '등록 실패: ' + (lastErr || '알 수 없는 오류'), 4200);
  });
