...
// ---- ⓘ 설명 ----
const NOTE_SEL = '.view-desc:not([data-keep]), .hint:not([data-keep]), [data-note]';

function infoBox() {
  const w = document.createElement('span');
  w.className = 'info';
  w.innerHTML = '<button type="button" class="info-i" aria-label="설명 보기" aria-expanded="false">ⓘ</button><span class="info-tip" role="tooltip"></span>';
  return w;
}

function pourNote(info, el) {
  ...
  if (el.matches('.warnbox, .notice')) {     // 상자째 설명이면 상자 모양을 살려 통째로
    el.removeAttribute('data-note');
    el.style.marginTop = '';
    tip.appendChild(el);
  } else {
    while (el.firstChild) tip.appendChild(el.firstChild);
    el.remove();
  }
}
...
export function foldNotes(root) {
  if (!root) return;
  const inline = new Map();                   // 제목 없는 범위 → 그 자리에 둔 ⓘ
  for (const el of [...root.querySelectorAll(NOTE_SEL)]) {
    if (!el.isConnected || el.closest('.info-tip')) continue;
    if (!el.textContent.trim()) { el.remove(); continue; }
    let anchor = null;
    if (el.matches('.view-desc')) {
      let p = el.previousElementSibling;
      while (p && !p.matches('.view-title')) p = p.previousElementSibling;
      anchor = p;
    }
    const scope = el.closest('.card, .modal') || root;
    if (!anchor) {
      const marks = [...scope.querySelectorAll('[data-note-anchor]')]
        .filter(a => a.compareDocumentPosition(el) & Node.DOCUMENT_POSITION_FOLLOWING);
      anchor = marks.pop() || null;
    }
    // 창(모달)은 foldNotes가 창 자체를 root로 받으므로 scope === root여도 제목을 찾아야 한다 —
    // 처음엔 'scope !== root'로 걸러서 창의 설명이 제목 옆이 아니라 제자리에 따로 붙었다.
    if (!anchor && scope.matches('.card, .modal')) anchor = scope.querySelector(scope.matches('.modal') ? 'h2' : 'h3');
    let info;
    if (anchor) {
      info = [...anchor.children].find(c => c.classList.contains('info'));
      if (!info) { info = infoBox(); anchor.appendChild(info); }
    } else {
      info = inline.get(scope);
      if (!info) {
        const holder = document.createElement('div');
        holder.className = 'info-inline';
        info = infoBox();
        holder.appendChild(info);
        el.before(holder);
        inline.set(scope, info);
      }
    }
    pourNote(info, el);
  }
...
// 말풍선은 **화면 기준 고정 위치**로 띄우고 좌표를 직접 계산한다.
function placeTip(info) {
  const tip = info.querySelector('.info-tip');
  const btn = info.querySelector('.info-i');
  if (!tip || !btn) return;
  const a = btn.getBoundingClientRect();
  const box = info.closest('.modal')?.getBoundingClientRect();
  const vw = document.documentElement.clientWidth;
  const vh = document.documentElement.clientHeight;
  const minL = Math.max(0, box ? box.left : 0) + 8;
  const maxR = Math.min(vw, box ? box.right : vw) - 8;
  tip.style.left = '0px';
  tip.style.top = '0px';
  const w = tip.offsetWidth, h = tip.offsetHeight;
  if (!w) return;                                    // 아직 안 보이는 상태
  let left = a.left - 8;
  if (left + w > maxR) left = maxR - w;              // 오른쪽으로 넘치면 왼쪽으로
  if (left < minL) left = minL;                      // 그래도 넘치면 왼쪽 끝에 맞춘다
  const below = a.bottom + 8;
  const up = below + h > vh - 8 && a.top - 8 - h >= 8;
  info.classList.toggle('up', up);
  tip.style.left = `${Math.round(left)}px`;
  tip.style.top = `${Math.round(up ? a.top - 8 - h : below)}px`;
}
// 고정 위치라 스크롤하면 ⓘ에서 떨어진다 — 따라가게 하고, ⓘ가 화면 밖으로 나가면 닫는다
function refreshTips() {
  for (const info of document.querySelectorAll('.info.open, .info:hover')) {
    const r = info.querySelector('.info-i')?.getBoundingClientRect();
    if (!r || r.bottom < 0 || r.top > document.documentElement.clientHeight) { closeTips(); return; }
    placeTip(info);
  }
}
window.addEventListener('scroll', refreshTips, true);   // true: 입력창 안 스크롤도 잡는다
window.addEventListener('resize', refreshTips);
function closeTips(except = null) {
  for (const x of document.querySelectorAll('.info.open')) {
    if (x === except) continue;
    x.classList.remove('open');
    x.querySelector('.info-i')?.setAttribute('aria-expanded', 'false');
  }
}
// 누르면 열고 닫는다. ⓘ가 링크·표 줄 안에 있어도 그쪽 동작(이동·행 열기)은 일으키지 않는다.
document.addEventListener('click', e => {
  ...
  if (btn) {
    ...
    if (open) placeTip(info);
    return;
  }
  if (e.target.closest?.('.info-tip')) return;   // 말풍선 안의 링크·글자 선택은 그대로
  closeTips();
}, true);
document.addEventListener('mouseover', e => { const info = e.target.closest?.('.info'); if (info) placeTip(info); });
document.addEventListener('keydown', e => { if (e.key === 'Escape') closeTips(); });

// ---- 입력칸 줄 맞춤 ----
const fieldControl = cell => cell.querySelector(
  ':scope > input:not([type=checkbox]):not([type=radio]):not([type=hidden]), :scope > select, :scope > textarea');

export function alignFields(root) {
  if (!root) return;
  const grids = root.matches?.('.form-grid') ? [root] : [...root.querySelectorAll('.form-grid')];
  for (const g of grids) {
    const cells = [...g.children].filter(c => !c.classList.contains('full') && fieldControl(c));
    for (const c of cells) c.style.paddingTop = '';
    const rows = new Map();                         // 윗변이 같은 칸 = 같은 줄
    for (const c of cells) {
      if (!c.offsetParent) continue;
      const key = Math.round(c.offsetTop);
      if (!rows.has(key)) rows.set(key, []);
      rows.get(key).push(c);
    }
    for (const row of rows.values()) {
      if (row.length < 2) continue;
      const offs = row.map(c => fieldControl(c).getBoundingClientRect().top - c.getBoundingClientRect().top);
      const max = Math.max(...offs);
      row.forEach((c, i) => { const d = max - offs[i]; if (d > 0.5) c.style.paddingTop = `${Math.round(d)}px`; });
    }
    // 라벨 글자가 바뀌거나(종목을 고르면 '가격' 옆 안내가 붙는 등) 화면 폭이 바뀌면 다시 맞춘다.
    if (!g._alignObs && 'ResizeObserver' in window) {
      let queued = false;
      g._alignObs = new ResizeObserver(() => {
        if (queued) return;
        queued = true;
        setTimeout(() => { queued = false; if (g.isConnected) alignFields(g); }, 0);
      });
      for (const c of cells) g._alignObs.observe(c);
    }
  }
}
...
export function openModal(html) {
  ...
  foldNotes(root.querySelector('.modal'));   // 호출부가 이벤트를 붙이기 전에 접는다
  alignFields(root.querySelector('.modal')); // 접은 뒤에 재야 설명이 빠진 자리 기준으로 맞는다
  // 호출부가 창을 연 직후 라벨 글자를 바꾸는 일이 흔하다(매도 창의 '최근 종가' 안내) — 그 뒤에 한 번 더
  setTimeout(() => alignFields(root.querySelector('.modal')), 0);
...
export function render() {
  ...
  foldNotes(main);   // 설명 문단을 ⓘ로 접는다 — bind_보다 먼저(옮긴 링크·버튼에도 이벤트가 붙게)
  alignFields(main); // 같은 줄 입력칸의 윗변을 맞춘다
  setTimeout(() => alignFields(main), 0);   // bind_가 라벨 글자를 바꾼 뒤에 한 번 더
  // 뷰별 후처리(이벤트 바인딩)
